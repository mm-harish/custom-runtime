...
struct alignas(64) TaskPool {
    ...
    std::vector<TaskType*> allocatedPointers; ///< Keeps track of allocated blocks for cleanup.
     ...
     */
    __attribute__((cold))
    inline std::pair<TaskType*, TaskType*> allocateTwoFrames() {
        TaskType* tasks = new TaskType[numframes];
        allocatedPointers.push_back(tasks);
        for (int i = 0; i < numframes; i++)
            front[i] = &tasks[i];
     ...
     */
    __attribute__((hot))
    inline std::pair<TaskType*, TaskType*> getTwoFrames() {
