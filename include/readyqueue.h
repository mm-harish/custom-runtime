 ...
 *
 * Maintains a local LIFO queue for the owner worker and a public FIFO queue for
 * stealing.
 *
 * @tparam FuncTy The type of the function identifier.
 */
template <typename FuncTy> struct alignas(64) ReadyQueue {
   ...
   */
  __attribute__((hot)) bool inline isLocalQueueFull() {
   ...
   */
  __attribute__((hot)) void local_push_back(TaskType *t) {
   ...
   */
  __attribute__((hot)) bool inline isLocalQueueEmpty() {
   ...
   */
  __attribute__((hot)) TaskType *local_pop_back() {
    if (localQueueBack == 0)
      return nullptr;
   ...
   * @param t The task to push.
   */
  __attribute__((hot)) void steal_push_back(TaskType *t) {
    if (back - front == n) {
      std::cout << "queue full\n";
      exit(0);
    }
  ...
  /**
   * @brief Pops a task from the steal queue (back) - used by owner when local
   * is empty.
   * @return The popped task, or nullptr if empty.
   */
  __attribute__((cold)) TaskType *steal_pop_back() {
    if (back == front) {
      return nullptr;
    }
